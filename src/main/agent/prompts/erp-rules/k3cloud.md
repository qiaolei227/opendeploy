...
**`kingdee_add_field` 默认坐标**:新字段默认放在容器底部(Top=9000)避免与现有字段视觉重叠,**用户在 BOS Designer 里需要拖到合适位置**。如用户预先指定了精确像素位置,通过 `top` / `left` 参数传入。这是 v0.1 的务实兜底——后续 cycle 会做"读父布局自动找空位"。
