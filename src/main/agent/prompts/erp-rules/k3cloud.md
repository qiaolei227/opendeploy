...
**单层树硬规则**:每个父对象(业务单据 / 基础资料 / 转换规则 / 其他支持扩展的元素)顶多挂 **1 个** 本项目 devCode 的 BOS 扩展。所有定制都堆到那一个扩展里 — **不允许新建并列的第二个扩展**。
...
- **候选 ≥ 1**:**静默复用**第一个候选,直接把它的 `extId` 传给后续工具(`k3cloud_add_fields` / `k3cloud_register_python_plugins` / `k3cloud_add_custom_operation` / `k3cloud_add_toolbar_button` 等)继续加东西。**不要再调 `k3cloud_create_extension`** — 调了也会被工具内 guard 拒绝并返回 existingExtId,白浪费一次工具调用。
- **候选 ≥ 2 (历史遗留)**:这种是过去版本反问"挂上去 vs 新建独立扩展"留下的,理论上不该出现。先复用第一个,跟用户提一下"父对象上有 N 个本项目扩展,建议整理合并"再继续 — 不阻断当前任务。

**v0.1 限制**:都只做**一级扩展**(直接继承原厂单据),不支持从已有扩展派生 2 级扩展。"再要扩展功能"=往同一个 1 级扩展里加更多字段 / 插件 / 操作,不是建第 2 层扩展。用户问"建二级扩展"就告知"v0.1 不支持多级扩展派生"。
