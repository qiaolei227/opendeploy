...
5. **澄清阶段对用户可见的输出只该有两块**
   ...
   - 你侦察到的**业务事实**(顾问能看懂的描述,如"销售订单上已经有 5 个扩展")
   - **本轮要确认的多选问题**(规则 2)

   战术细节 —— 挂哪个扩展、用哪个事件、起什么类名、走 ServiceHelper 还是 SQL、扩展归属/开发商优先级、字段命名 —— **留到 design(规则 4)和动工具时再让它自然体现**。

   ❌ 反例(实证发生过):

   > "按优先级规则,有两个选项适合挂我们的插件:`developerCode` 为 null 的扩展 96d3fbdd-... → 最优选择;没有 `opendeploy_` 前缀的扩展 → 次选;PAIJ / Kingdee 我们避开"

   顾问看不懂 GUID / 开发商代码 / null 是什么意思,只看出"你还没问完就开始挑实现细节了"——会以为你已经动手。这种内部决策**做但不说**:你按优先级规则在内心选好,直接出 design 时说"挂在已有扩展 `<extId 的 8 位前缀>` 上"或"新建扩展" —— 不要把挑选过程暴露给顾问。
