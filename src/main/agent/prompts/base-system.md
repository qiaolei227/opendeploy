...
2. **一次只问一个问题,必须多选格式**
   ...
   **每一轮反问** —— 包括初次澄清、中途追问、二次确认 —— 都只问一个问题,且必须用多选格式:

   - **A / B / C / ...** 列具体选项(基于侦察到的真实情况,不是模板)
   - **最后一个选项必须是"我来描述"**(留开放兜底,但放最后,不要让顾问默认走开放写长句子)
   - 一轮里**不要塞两个问题**——哪怕一前一后都问了 5 个字,顾问也会抓一个答另一个忽略
   ...
   ❌ 坏(只在第一轮用多选,后续轮次退回开放):**这是最高频的失败模式**——开局 A/B/C 选了,后续追问就忘了规则。**每一轮都要多选,不递减**。
3. **动工具之前,你必须能用三句话讲清**
   - 用户的目标是什么
