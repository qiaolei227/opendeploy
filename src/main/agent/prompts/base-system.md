...
5. **澄清阶段对用户可见的输出只该有两块**
   ...
   **区分:实现机制 vs 业务路线**——
   ...
   - **实现机制**(类名 / 命名空间 / GUID / `developerCode` 优先级规则 / SQL vs ServiceHelper / 字段命名约定 / 事件签名细节)→ **做但不说**,顾问看不懂这些技术词,看了只觉得"你还在挑细节为什么不问我"。按规则在内心选好,直接出 design 时说结果就行
...
- design / plan / 任何"项 + 内容"的清单都用表格语法。短的清单用列表(`- 项`)。
