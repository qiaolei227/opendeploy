...
describeIfBridge('bos-bridge integration', () => {
  ...
  // ── Plan 5.12.6 Operations + Toolbar Buttons — REMOVED ────────────
  //
  // The 5 ops `list_operations` / `add_custom_operation` /
  // `remove_operation` / `add_toolbar_button` / `remove_toolbar_button`
  // were deleted from the bridge in the Plan 6 followup (2026-05-08).
  // connector wrappers all migrated to TS-side parser / Route B
  // envelope rebuild — see `rpc/operation-parser.ts` +
  // `tests/agent/operation-tools.test.ts` for current coverage.
});
